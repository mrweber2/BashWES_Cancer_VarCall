...
## choose the run case
  ANALYSIS=<depending on the analysis type it can be {ANALYSIS=MUTECT_VC, or ANALYSIS=VC_WITH_MUTECT, or ANALYSIS=MUTECT} for variant calling excluding local realignment with MuTect2, {ANALYSIS=ALIGNMENT, or ANALYSIS=ALIGN or ANALYSIS=ALIGN\_ONLY} for alignment only, {ANALYSIS=VC\_WITH\_REALIGNMENT} for complete variant calling with realignment, or anything else for complete variant calling without realignment>
