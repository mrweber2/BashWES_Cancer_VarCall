...
#-----------------------------------------------------------------------------------------------
## MANIFEST and COMMAND CHECK
#-----------------------------------------------------------------------------------------------

## Set manifest and error log name
...
ERRLOG=error.log
echo -e "${MANIFEST}" >> ${ERRLOG}

set -x
#redmine=hpcbio-redmine@igb.illinois.edu

if [[ $# != 1 ]]  ## If running command without 1 argument, print this error and exit
then
...
if [ ! -s ${runfile} ] ## If runfile empty or non-existent, print this error and exit
then
...
then
	...
	then
		MSG="Incorrect value for ALIGNERTOOL=$alignertool in the runfile."
		echo -e "program=$0 stopped at line=${LINENO}. Reason=${MSG}" >> ${ERRLOG}
		exit 1;
	fi
fi
...
#-----------------------------------------------------------------------------------------------
## SETUP/CONFIGURE OUTPUT FOLDERS AND FILES
#-----------------------------------------------------------------------------------------------
...
if [[ ! -d ${outputdir}/logs ]]
then
	mkdir -p ${outputdir}/logs ## The output directory does not exist. Create it.
fi

if [[ ! -d ${outputdir}/${deliverydir}/docs ]]
then
	mkdir -p ${outputdir}/${deliverydir}/docs ## The delivery directory does not exist. Create it.
fi

if [[ ! -d ${outputdir}/${deliverydir}/jointVCFs  ]]
then
	mkdir -p ${outputdir}/${deliverydir}/jointVCFs ## The jointVCF directory (containing files before VQSR) does not exist. Create it.
fi
...
## Create and populate qsub with PBS options
...
do
	...
	then
		...
		echo -e "\n\n#-----------------------------------------------------------------------------------------------\nProcessing next line ${sampleLine}...\n##### col1=sample_name col2=read1 col3=read2  including full paths\n##### sample_name will be used for directory names and in RG line of BAM files\n#-----------------------------------------------------------------------------------------------\n\n"
...
#-----------------------------------------------------------------------------------------------
## CREATING ANISIMOV BUNDLE FOR ANALYSIS OF ALL SAMPLES
#-----------------------------------------------------------------------------------------------

## Calculate the number of nodes needed, to be numbers of samples +1
numnodes=$((inputsamplecounter+1))

## Form the qsub script
analysisqsub=${TopOutputLogs}/qsub.${analysis}
cat ${generic_qsub_header} > ${analysis}qsub

echo "#PBS -N ${analysis}.${batchname}" >> ${analysis}qsub
echo "#PBS -o ${TopOutputLogs}/log.${analysis}.ou" >> ${analysis}qsub
echo "#PBS -e ${TopOutputLogs}/log.${analysis}.er" >> ${analysis}qsub
echo "#PBS -l nodes=${numnodes}:ppn=${thr}" >> ${analysis}qsub
echo -e "\n" >> ${analysis}qsub
echo "aprun -n ${numnodes} -N 1 -d ${thr} ~anisimov/scheduler/scheduler.x ${TopOutputLogs}/Anisimov.${analysis}.joblist /bin/bash > ${TopOutputLogs}/Anisimov.${analysis}.log" >> ${analysis}qsub
echo -e "\n" >> ${analysis}qsub
echo "cat ${outputdir}/logs/mail.${analysis}.SUCCESS | mail -s \"[Task #${reportticket}]\" \"${redmine},${email}\" " >> ${analysis}qsub
echo "cat ${outputdir}/logs/mail.${analysis}.FAILURE | mail -s \"[Task #${reportticket}]\" \"${redmine},${email}\" " >> ${analysis}qsub

`chmod ug=rw ${TopOutputLogs}/Anisimov.${analysis}.log`
`chmod ug=rw ${analysis}qsub`

analysisjobid=`qsub ${analysis}qsub` 

`qhold -h u ${analysisjobid}`

echo ${analysis}jobid >> ${TopOutputLogs}/pbs.${analysis}
echo ${analysis}jobid >> ${TopOutputLogs}/pbs.summary_dependencies
echo `date`

if [[ `expr ${#analysisjobid}` -lt 1 ]]
then
	MSG="unable to launch ${analysis} qsub job ${analysis}jobid. Exiting now"
	echo -e "Program $0 stopped at line=${LINENO}.\n\n${MSG}" >> ${ERRLOG}
	exit 1;        
fi
